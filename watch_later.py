...
class WatchLaterPlugin(GObject.Object, Peas.Activatable):
    ...
    def __init__(self):
        ...
        self.base_dir, file = os.path.split(os.path.normpath(
            os.path.realpath(os.path.normcase(os.path.expandvars(
    ...
    def do_activate(self):
        ...
        if self.restart_last:
            self.restart_last_timer = Timer(self.restart_delay,
                                            self.restart_last_played)
    ...
    def on_file_played(self, obj, file: str):
        ...
        if file != self.file:
            raise RuntimeError(f'The opened file {self.file} and the played ' +
    ...
    def on_file_closed(self, obj):
        ...
        if self.current_time > 0 and \
                self.current_time >= self.min_runtime + self.rewind_time and \
                self.current_time < self.stream_length - self.max_runtime:
        ...
        if save_time > 0 and os.path.exists(unquoted_file):
            ...
            with open(self.last_played_file, 'w', encoding='utf8') as f:
                f.write(f'{self.file}\n')
        elif (save_time == 0 or not os.path.exists(unquoted_file)) and \
                self.watch_later_file is not None and \
                os.path.exists(self.watch_later_file):
    ...
    def go_to_last_time(self):
        ...
        if self.current_time > 0:
            GLib.timeout_add(50, go_to_last_time_thread, self._totem,
                             self.current_time)
    ...
    def update_properties(self, start_timer=False):
        ...
        if start_timer:
            self.properties_timer = Timer(self.update_interval,
                                          self.update_properties, [True])
    ...
    def restart_last_played(self):
        ...
        if os.path.exists(self.last_played_file):
            with open(self.last_played_file, 'r', encoding='utf8') as f:
                self._totem.remote_command(Totem.RemoteCommand.REPLACE,
                                           f.read().strip())
